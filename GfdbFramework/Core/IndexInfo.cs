...
namespace GfdbFramework.Core
{
    ...
    {
        /// <summary>
        /// 使用指定的索引名称、索引字段集合、索引类型以及排序方式初始化一个新的 <see cref="IndexInfo"/> 类实例。
        /// </summary>
        /// <param name="name">该索引的名称。</param>
        /// <param name="fields">该索引应用的字段集合。</param>
        /// <param name="type">该索引的类型。</param>
        /// <param name="sort">该索引的排序方式。</param>
        internal IndexInfo(string name, ReadOnlyList<OriginalField> fields, IndexType type, SortType sort)
        {
            Name = name;
            Fields = fields;
            Type = type;
        ...
        /// 获取该索引应用的字段集合。
        /// </summary>
        public ReadOnlyList<OriginalField> Fields { get; }
